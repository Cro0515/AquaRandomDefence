...
public class PufferTowerMgr : MonoBehaviour
{
    ...
    [HideInInspector] public float  m_FreezeTime;   //슬로우(디버프) 지속시간
    ...
    void Awake () {
        ...
        m_BulletObj = Resources.Load<GameObject>("Prefab/Bullet/Puffer_Bullet");
    ...
    void Start()
    {
        //타워 정보 가져오기
        m_Grade = Grade.Rare;
        m_Damage = TowerStatus.PufferTower.m_Damage[(int)m_Grade];
        m_Range = TowerStatus.PufferTower.m_Range[(int)m_Grade];
        m_AttackSpped = TowerStatus.PufferTower.m_AttackSpeed[(int)m_Grade];
        m_FreezeTime = TowerStatus.PufferTower.m_FreezeTime[(int)m_Grade];
        //
