...
public class PoisonPufferTowerMgr : MonoBehaviour
{
    ...
    [HideInInspector] public float  m_PoisonTime;       //슬로우(디버프) 지속시간
    [HideInInspector] public int    m_PoisonDamage;     //독 데미지
    ...
    void Awake () {
        ...
        m_BulletObj = Resources.Load<GameObject>("Prefab/Bullet/PoisonPuffer_Bullet");
    ...
    void Start()
    {
        ...
        //타워 정보 가져오기
        m_Grade = Grade.Rare;
        m_Damage = TowerStatus.PoisonPufferTower.m_Damage[(int)m_Grade];
        m_Range = TowerStatus.PoisonPufferTower.m_Range[(int)m_Grade];
        m_AttackSpped = TowerStatus.PoisonPufferTower.m_AttackSpeed[(int)m_Grade];
        m_PoisonTime = TowerStatus.PoisonPufferTower.m_PoisonTime[(int)m_Grade];
        m_PoisonDamage = TowerStatus.PoisonPufferTower.m_PoisonDamage[(int)m_Grade];
        //
    ...
    void AttackTarget () {
        ...
        */
        ...
        int rndNumber = Random.Range(0, m_TargetList.Count);
        
        LinkedListNode<GameObject> RandomTarget = m_TargetList.First;
        if(rndNumber != 0)
            for (int i = 0; i < rndNumber; i++) {
                RandomTarget = RandomTarget.Next;
            }

        SoundMgr.g_inst.EffectPlay(SoundMgr.g_inst.PufferAttack_Sound);


        //총알 발사
        //오브젝트 풀에서 총알 꺼낸뒤, 총알 스크립트의 스폰함수로 총알 생성
        BulletCtrl a_BulletSC = ObjPool_Bullet.PopObject().GetComponent<BulletCtrl>();
        a_BulletSC.BulletSpawn(RandomTarget.Value, this.gameObject.transform.position, ObjPool_Bullet);
        //
